...
int translateFunctionFromSymbol( unsigned int address, char *func )
{
  ...
  int  len;
  int i;
  sprintf( line, "addr2line -e %s -f -C 0x%x", imageName, address );
...
void emitSymbols( void )
{
  ...
  for (from = 0 ; from < MAX_FUNCTIONS ; from++) 
  {
    ...
    if (totals[from]) 
    {
      ...
      fprintf( fp, "  node%d [ label=\"%s\",shape=\"rectangle\"]\n", from,functions[from].funcName );

    } 
    else 
    {

      fprintf( fp, "  node%d [ label=\"%s\", shape=\"ellipse\"]\n",from, functions[from].funcName );
  ...
  for (from = 0 ; from < MAX_FUNCTIONS ; from++) {
    ...
    for (to = 0 ; to < MAX_FUNCTIONS ; to++) {
      ...
      if (calls[from][to]) {
        fprintf( fp, "  node%d -> node%d [label=\"%d \" fontsize=\"10\"]\n", 
                  from, to,
                  calls[from][to] );
      }
